    ...
    <div className="min-h-screen bg-slate-50 py-24 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {videosData.map((video: any, idx: number) => {
            const src: string = video.url ?? video.file ?? "";
            const key = video.id ?? src ?? idx;
            const isMp4 = src.toLowerCase().endsWith(".mp4");

            return (
              <div key={key} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-200 flex flex-col">
                ...
                <div className="p-6">
                  <span className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-2 block">
                    Video: {video.id ?? src.split("/").pop() ?? "unknown"}
                  </span>
